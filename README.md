...
RTX 5070 Ti 에서 학습하고, 엣지 디바이스(Hailo-8L) 배포를 염두에 두고 `imgsz=640` 으로 통일했다.
...
| [`rake_damage/`](rake_damage/README.md) | **레이크 파손 검출** | 객체 검출 (bbox) | `yolov8n` | `damage` | 38 / 8 |
...
레이크 파손부를 bbox 로 검출한다. 영상 프레임 누수를 막는 **시간순 분할**,
결과 영상 생성, **ONNX export(Hailo 배포용)** 까지 포함한다.
